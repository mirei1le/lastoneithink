...
<head>
    ...
    <title>Decor Products - Shopaholics</title>
...
<div class="product-container">
    ...
    if (isset($_GET['search'])) {
        ...
        $sql = "SELECT p.* FROM products p
                JOIN subcategories s ON p.subcategory_id = s.subcategory_id
                WHERE s.subcategory_name = 'Decor'";
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            echo "</form>";
            echo "<a href='decor.php?product_id=" . htmlspecialchars($row['product_id']) . "#reviews' class='view-reviews'>View Reviews</a>";
            echo "</div>";
            echo "</div>";
        }
    } else {
        if (isset($_GET['search'])) {
            ...
            echo "<p>No decor products found.</p>";
        }
    }
    ?>
</div>
