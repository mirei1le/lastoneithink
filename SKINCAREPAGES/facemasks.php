...
<head>
    ...
    <title>Face Masks - Shopaholics</title>
    ...
    <div class="product-container">
        ...
        if (isset($_GET['search'])) {
            ...
            $sql = "SELECT p.* FROM products p
                    ...
                    AND s.subcategory_name = 'Face Masks'";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ss", $searchParam, $searchParam);
            $stmt->execute();
            $result = $stmt->get_result();
            
            echo "<h2 class='search-results-title'>Search Results for '" . htmlspecialchars($searchTerm) . "' in Face Masks</h2>";
        } else {
            $sql = "SELECT p.* FROM products p
                    JOIN subcategories s ON p.subcategory_id = s.subcategory_id
                    WHERE s.subcategory_name = 'Face Masks'";
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                echo "</form>";
                echo "<a href='facemasks.php?product_id=" . htmlspecialchars($row['product_id']) . "#reviews' class='view-reviews'>View Reviews</a>";
                echo "</div>";
                echo "</div>";
            }
        } else {
            if (isset($_GET['search'])) {
                echo "<p class='no-results'>No face masks found matching your search.</p>";
            } else {
                echo "<p>No face masks available at the moment.</p>";
            }
        }
        ?>
    </div>
