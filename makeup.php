...
<head>
    ...
    <title>Makeup Products - Shopaholics</title>
...
<div class="banner-container">
    <img src="IMGM/Makeup_Banner.jpg" alt="Makeup Banner" class="banner-image">
</div>
...
<div class="product-container">
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<div class='product-image' onclick='showProductModal(" . json_encode($row) . ")'>";
            echo "<img src='IMGM/" . htmlspecialchars($row['image']) . "' alt='" . htmlspecialchars($row['name']) . "'>";
            echo "</div>";
            echo "<div class='product-details'>";
            echo "<p class='product-brand'>$category</p>";
            echo "<h3 class='product-title' onclick='showProductModal(" . json_encode($row) . ")'>" . htmlspecialchars($row['name']) . "</h3>";
            echo "<p class='product-price'>€" . htmlspecialchars($row['price']) . "</p>";
            echo "<button class='add-to-bag' onclick='showProductModal(" . json_encode($row) . ")'>VIEW DETAILS</button>";
            echo "<a href='makeup.php?product_id=" . htmlspecialchars($row['product_id']) . "#reviews' class='view-reviews'>View Reviews</a>";
            echo "</div>";
            echo "</div>";
        }
    } else {
    ...
    <div class="modal-content">
        ...
        <div class="product-header">
            ...
            <div class="product-info">
                ...
                <p class="modal-note">Quantity can be adjusted above</p>
            </div>
        </div>
    </div>
</div>
