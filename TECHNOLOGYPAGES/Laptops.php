...
<head>
    ...
    <title>Laptops - Shopaholics</title>
...
<!-- HEADER & NAVIGATION -->
<header>
    <h3 class="promo"> USE CODE [NEW2025] FOR EXTRA UP TO 15% OFF SELECT LAPTOPS </h3>
</header>
...
<!--<img src="../IMG/LaptopsBanner.jpg" alt="Laptops Banner" class="banner-image">-->
...
<div class="product-container">
    ...
    if (isset($_GET['search'])) {
        ...
        $sql = "SELECT * FROM products 
                ...
                AND category_id = 1 
                AND subcategory_id = (SELECT subcategory_id FROM subcategories WHERE subcategory_name = 'Laptops')";
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            echo "</form>";
            echo "<a href='Laptops.php?product_id=" . htmlspecialchars($row['product_id']) . "#reviews' class='view-reviews'>View Reviews</a>";
            echo "</div>";
            echo "</div>";
        }
    } else {
        if (isset($_GET['search'])) {
            echo "<p class='no-results'>No laptops found matching your search.</p>";
        } else {
            echo "<p>No laptops currently available. Check back soon!</p>";
        }
    }
    ?>
</div>
